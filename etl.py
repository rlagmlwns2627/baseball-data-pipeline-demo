...
ETL 파이프라인 메인 스크립트
실제 회사 파이프라인 구조:
  FTP 수신 → CSV 병합 → 모델 로드 → 파생변수 생성 → 매핑 → 최종 결과 저장
"""
...
# ─────────────────────────────────────────
# 경로 설정
# 로컬 실행 시 BASE_PATH 기본값 사용, Docker 실행 시 환경변수로 덮어씀
# ─────────────────────────────────────────
...
# ─────────────────────────────────────────
# STEP 1. 개별 CSV 파일 병합 (FTP 수신 데이터)
# team_info.csv가 같은 폴더에 있으므로 game_*.csv 패턴으로 한정
# ─────────────────────────────────────────
print("\n[STEP 1] 개별 CSV 파일 병합 중...")

csv_files = glob.glob(os.path.join(INPUT_PATH, "game_*.csv"))

if not csv_files:
    raise FileNotFoundError(f"'{INPUT_PATH}' 폴더에 game_*.csv 파일이 없습니다.")
...
# ─────────────────────────────────────────
# STEP 2. 머신러닝 모델 로드 → 파생변수 생성
# ─────────────────────────────────────────
print("\n[STEP 2] 모델 로드 및 파생변수 생성 중...")
